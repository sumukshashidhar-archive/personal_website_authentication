...
module.exports = {
  serviceError: function (message, service) {
    logger.error(
      `Received internal service error with ${service} with message ${message}`
    );
  ...
  generalMessage: function (message, service, status) {
    ...
    logger.silly(
      `Received a message request with ${message} and service ${service} with status ${status}`
    );
    return {
      message: message,
      status: status,
  ...
  objectReturn: function (message, service, status, object) {
    ...
    logger.silly(
      `Received a object return request with ${message} at service ${service} and status ${status} with object ${object}`
    );
    return {
      message: message,
      status: status,
  ...
  externalMessage: function (message, view) {
    logger.silly(
      `Received an external message request with ${message} at endpoint ${view}`
    );
    return {
      message: message,
    };
